"""
Главный модуль Telegram бота - Crypto Signals Bot
Теперь использует модульную структуру из modules/
"""
import logging
import asyncio
import sys
...
from modules.market_analyzer import analyzer, scan_market_signals, get_pocket_option_asset_name, get_expiration_time
...
**📞 Поддержка:** {SUPPORT_CONTACT}"""
    ...
    async def cmd_short(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Получение сигнала
        try:
            signals = await scan_market_signals('short')
            ...
            if signals:
                ...
                pocket_asset = get_pocket_option_asset_name(asset_name)
                
                payout = signal_info.get('payout', 85)
                payout_info = f"💎 Высокая доходность: {payout}%" if payout >= 92 else ""
                
                signal_text = f"""{signal_emoji} **SHORT СИГНАЛ**

📊 **Актив:** {pocket_asset}
📈 **Направление:** {signal_info['signal']}
⏱ **Экспирация:** {get_expiration_time(timeframe)}
...
💰 **Регистрация:** [Pocket Option]({POCKET_OPTION_REF_LINK})"""
    ...
    async def cmd_long(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            signals = await scan_market_signals('long')
...
/broadcast <message> - Рассылка"""
    ...
    def run(self):
        ...
        logger.info("📦 Используется модульная структура:")
        logger.info("   - modules/constants.py")
        logger.info("   - modules/market_analyzer.py")
        logger.info("   - modules/database_extended.py")
